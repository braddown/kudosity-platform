    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data) {
          ...
          if (result.data.url) responseText += `• URL: ${result.data.url}\n`;
          if (result.data.events) responseText += `• Events: ${result.data.events.join(', ')}\n`;
          if (result.data.name) responseText += `• Name: ${result.data.name}\n`;
          if (result.data.active !== undefined) responseText += `• Status: ${result.data.active ? 'Active' : 'Inactive'}\n`;
        }
